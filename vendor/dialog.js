    ...
    if (image_fond == null) {
        ...
        image_fond.onload = function () {
            dialog_dim_carte.querySelector(".hauteur").value =
                Math.round(dialog_dim_carte.querySelector(".largeur").value * image_fond.height / image_fond.width);
        }
    }
...
function afficher_Details(col, row) {
    ...
    if (m_selected === null || typeof m_selected === "undefined") {
        ...
        if (p_selected.Is_joueur) {
            ...
            dialog_details_2.querySelector("#indice").style.visibility = "hidden";
            indice.closest("td").style.visibility = "hidden";
            dialog_details_2.querySelector("#Dupliquer").disabled = true;
        }
        else {
            // Affichage de l'indice pour les PNJ
            dialog_details_2.querySelector("#indice").style.visibility = "";
            indice.closest("td").style.visibility = "";
...
function affiche_def() {
    ...
    else if ((defenseur.pr1_def && marge > -2) || (defenseur.pr2_def && marge > -2) || (defenseur.esq_def && marge > -4)) {
        dialog_defense_2.querySelector(".dommages").innerHTML =
...
function new_loc() {
    ...
    let A_distance = null;
    if (attaquant.at1_att) A_distance = Armes.find(w => w.Nom === attaquant.Arme1).A_distance;
    if (attaquant.at2_att) A_distance = Armes.find(w => w.Nom === attaquant.Arme2).A_distance;
...
dialog_dim_carte.querySelector(".hauteur").addEventListener("input", function (event) {
    if (image_fond === null) return;
    dialog_dim_carte.querySelector(".largeur").value =
        Math.round(event.target.value * image_fond.width / image_fond.height);
});
...
for (let i = 0; i < inputs.length; i++) {
    inputs[i].addEventListener("input", function (event) {
        // Conversion du nom de classe en nom de propriété
        const field = event.target.className.charAt(0).toUpperCase() +
...
function info_arme(arme) {
    ...
    if (arme === 1 && (!m_selected.Arme1 || m_selected.Arme1 === "Lancement de sort" || m_selected.Arme1 === "Bouclier")) {
        return "-";
    }
    if (arme === 2 && (!m_selected.Arme2 || m_selected.Arme2 === "Lancement de sort" || m_selected.Arme2 === "Bouclier")) {
        return "-";
    }
    ...
    if (m_selected.Arme1 && m_selected.Arme1 !== "" && m_selected.Arme2 && m_selected.Arme2 !== "") {
...
dialog_details_2.querySelector(".arme1").addEventListener("change", function (event) {
    ...
    // Gestion des armes normales
        ...
        if (event.target.value !== "Bouclier") {
            nouvelleOption = document.createElement("option");
            nouvelleOption.value = "Bouclier";
            nouvelleOption.textContent = "Bouclier";
            arme2.appendChild(nouvelleOption);
    ...
    m_selected.Arme1 = event.target.value;
...
dialog_details_2.querySelector(".type").addEventListener("change", function (event) {
    ...
    // Couleur selon le type
    if (event.target.value === "allies") event.target.style.backgroundColor = "rgb(192, 255, 192)";
